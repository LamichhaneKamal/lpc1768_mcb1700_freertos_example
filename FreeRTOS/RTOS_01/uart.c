...
void UART_Init(uint32_t var_baudRate_u32)
{
    ...
    LPC_UART0->FCR = M_UartFifoControl_U32;		// Enable FIFO and reset Rx/Tx FIFO buffers	
	LPC_UART0->LCR = M_UartLineControl_U32;	    // 8bit data, 1Stop bit, No parity
...
void UART_SetBaudRate(uint32_t var_baudRate_u32)
{
	uint32_t var_RegValue_u32, var_Pclk_u32;

	
	var_Pclk_u32 = uart_GetPclk();
   	...
   	if((var_baudRate_u32 >= C_MinBaudRate_U32) && (var_baudRate_u32<=C_MaxBaudRate_U32))
	{
		...
		var_RegValue_u32 = M_GetBaudRateGeneratorValue(var_Pclk_u32,var_baudRate_u32);
	}
	else
	{
		/*	 Invalid baudrate requested, hence set it to default baudrate of 9600 */
		var_RegValue_u32 = M_GetBaudRateGeneratorValue(var_Pclk_u32,9600);
	}
		
	LPC_UART0->DLL = util_ExtractByte0to8(var_RegValue_u32);
	LPC_UART0->DLM = util_ExtractByte8to16(var_RegValue_u32);
...
/***************************************************************************************************
							void uart_GetPclk(uint32_t var_Pclk_u32)
 ***************************************************************************************************
 * I/P Arguments: uint32_t : var_Pclk_u32 
 * Return value	: var_Pclk_u32 in terms of system frequency
...
static uint32_t uart_GetPclk(void)
{
    uint32_t var_UartPclk_u32,var_Pclk_u32, SystemFrequency;
	/** 
...
char UART_RxChar()
{
	...
	while(util_IsBitCleared(LPC_UART0->LSR,M_UartRDRposition_U8));  	// Wait till the data is received
	data = LPC_UART0->RBR;                          	// Read received data	
...
void UART_TxChar(char var_uartData_u8)
{
	...
	while(util_IsBitCleared(LPC_UART0->LSR,M_UartTHREposition_U8));	  /* Wait till transmitter buffer becomes empty */
    LPC_UART0->THR=var_uartData_u8;						// Load the data to be transmitted    
}
